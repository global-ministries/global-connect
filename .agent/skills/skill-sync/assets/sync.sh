...
# Auto-run setup.sh to sync all agent files (unless dry-run)
if [ "$DRY_RUN" = false ]; then
    echo ""
    echo -e "${BLUE}Auto-syncing to all AI agents...${NC}"
    
    SETUP_SCRIPT="$REPO_ROOT/skills/setup.sh"
    if [ -f "$SETUP_SCRIPT" ]; then
        if "$SETUP_SCRIPT" --all > /dev/null 2>&1; then
            echo -e "${GREEN}✓ Successfully synced AGENTS.md to all agents${NC}"
            echo -e "  ${CYAN}Updated: GEMINI.md, CLAUDE.md, .agent/rules/agent.md, .github/copilot-instructions.md${NC}"
        else
            echo -e "${YELLOW}⚠ Warning: setup.sh failed. Run manually: ./skills/setup.sh --all${NC}"
        fi
    else
        echo -e "${YELLOW}⚠ setup.sh not found. Skipping auto-sync.${NC}"
    fi
fi
