 ...
 * client.tsx with the same two write surfaces (both gated behind
 * `canWrite`, re-checked by the server actions + RLS):
 ...
 *
 * Restyled onto sistema-diseno primitives (BotonSistema, 44px touch
 * targets) instead of the old page's raw Tailwind buttons — see this
 * task's brief. Behavior is unchanged: toggles stay optimistic (the
 * checkbox flips immediately and reverts on error).
 */

import { useState, useTransition, type ReactElement } from 'react'
import { useRouter } from 'next/navigation'

import { TarjetaSistema, TextoSistema, BotonSistema } from '@/components/ui/sistema-diseno'
        ...
        {talleres.length === 0 ? (
          ...
          <ul className="grid gap-2">
            {talleres.map((taller) => {
              ...
              return (
                <li key={taller.id}>
                  <label className="flex min-h-[44px] items-center gap-3 rounded border p-3">
                    <input
                      type="checkbox"
                      checked={checked}
