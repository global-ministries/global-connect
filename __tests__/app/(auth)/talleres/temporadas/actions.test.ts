 ...
 *
 * T8 (odd/tasks/talleres-consolidar-pantallas.md) — server actions for
 * /talleres/temporadas, ported unchanged from the old
 * app/(auth)/admin/talleres/temporadas/actions.ts (behavior, validation and
 * the write gate are identical — only the module's location and its
 * revalidatePath targets move). See that action's own header for the
 * gate's evidence: talleres_temporadas' INSERT/UPDATE/DELETE RLS policies
 * (supabase/migrations/20260819000001_pr45_talleres_temporadas.sql) call
 * the UNSCOPED `auth_has_talleres_capability` — any director.write or
 * admin.manage grant, anywhere, satisfies it — so this flat capability
 * check is not an approximation, it is the exact mirror of the base's own
 * gate (the same reasoning T2's `puedeCrear` documents for "crear taller").
 *
