...
- [ ] 4.1 Update `app/(auth)/grupos-vida/mapa/page.tsx` and `components/grupos-vida/mapa-*.tsx` to use host-home RPC, filters, and neutral privacy copy.
...
- [ ] 4.3 Add member-layer rendering/tests in `components/grupos-vida/mapa-*.tsx`; verify unauthorized users receive no member payload.
