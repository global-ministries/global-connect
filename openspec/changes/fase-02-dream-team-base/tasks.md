...
- [ ] 4.2 DT-013 — Aplicar migración a `supabase_global_staging` vía MCP tool `supabase_global_staging_apply_migration`. **BLOCKER**: MCP `supabase_global_staging` retorna `Connection terminated due to connection timeout` para cualquier query/migration. Migración creada en archivo pero NO aplicada a staging todavía. Validar que 7 tablas + 5 enums existen (`supabase_global_staging_list_tables`), RLS bloquea authenticated sin capability, y helper `auth_has_dream_team_capability` funciona. Archivos: N/A (operación MCP). Verificación: `supabase_global_staging_list_tables` muestra `dream_team_*`; query de smoke RLS retorna denied para usuario sin capability.
