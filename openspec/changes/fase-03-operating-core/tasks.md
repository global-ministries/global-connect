...
- [x] **S00** mobile-nav timeout + drop `coverageThreshold`, `I`, `type:bug`, `F(mobile-platform-navigation)` + `pnpm test:ci`, `UI`, revert=baseline, ~120
...
- [x] **S16** Resources contract+repo+API, `I`, `type:feature`, `F(OC+api/resources)`, `N/A`+`HTTP`+`R`, revert=resource-route+repo, ~280
...
- [x] **S18** `*.v1` Spanish templates, `I`, `type:feature`, `F(emails/OC-templates)`, `N/A`, revert=unused, ~280
...
- [x] **S23** Flags/route-access + rollout (signature guard by parameter NAMES), `I`, `type:feature`, `F(flags-route-access-compat-e2e)`, `OFF`+`R`, revert=kill-switch OFF, ~220
