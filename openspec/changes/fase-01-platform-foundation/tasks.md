...
- [ ] 1.1 Crear `lib/platform/preflight.ts` para bloquear `uno_a_uno` sin baseline/archive/reintroducción formal; test de falla pre-implementación.
- [ ] 1.2 Crear `lib/platform/session/types.ts` y builder desde `auth.uid()`/server session; rechazar `personaId` cliente como identidad; tests.
- [ ] 1.3 Crear `lib/platform/persona.ts` para búsqueda/dedupe con minimización, masking, auditoría, boundaries y tests anti-enumeración.
- [ ] 1.4 Crear `lib/platform/experiences.ts` y scopes allowlisted; scopes ausentes/malformados/desconocidos/duplicados/conflictivos fallan cerrado con tests.
...
## Fase 5: Ledger longitudinal

- [ ] 5.1 Crear `lib/platform/participation.ts` con clasificación de sensibilidad, retención, actor/fuente y boundaries de lectura.
- [ ] 5.2 Probar lectura autorizada, denegación fuera de scope, evento sin permisos y no revelación de existencia de datos sensibles.

## Fase 6: Grants, observabilidad y rollout

- [ ] 6.1 Crear `lib/platform/grants.ts` con auditoría actor/fuente/before-after/deny, logs de denegación, métricas y alertas.
- [ ] 6.2 Exigir por PR: tests del slice, build verde, umbral de error/denegación productiva, rollout staged y ruta fix-forward/rollback al legado.
