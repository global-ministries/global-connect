-- Extend the external inbound support persistence RPC to persist callback action visibility.
-- Additive migration: replaces the existing function signature with internal/external flag.

DROP FUNCTION IF EXISTS public.record_support_external_inbound_update(uuid, uuid, text, text);
...
BEGIN
  ...
  VALUES (p_ticket_id, p_author_usuario_id, p_message_body, p_is_internal)
