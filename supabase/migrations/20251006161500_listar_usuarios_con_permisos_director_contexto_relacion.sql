-- Extiende p_contexto_relacion a director-etapa (visibilidad global temporal en contexto relaciones)
  ...
  CASE usuario_rol
    ...
    WHEN 'director-etapa' THEN
      IF p_contexto_relacion THEN
        -- Igual que líder: visibilidad amplia sólo en contexto relaciones
        query_where := ' WHERE 1=1 ';
      ELSE
        query_where := format('
          WHERE u.id IN (
