-- Migración: Ajustar umbral de grupos en riesgo
-- Cambios:
-- - top_5_grupos_en_riesgo ahora filtra solo grupos con < 70% de asistencia
-- - grupos_en_riesgo_todos filtra < 70%
-- - Excluir explícitamente grupos con 100% del riesgo
...
BEGIN
  ...
  -- Grupos en riesgo (TOP 5, solo < 70% y > 0%)
  v_grupos_riesgo := (
    WITH grupos_perm AS (
      SELECT g.id
      FROM public.grupos g
...
IS 'Reporte semanal de asistencia. top_5_grupos_en_riesgo filtra < 70% (excluye 0%). grupos_en_riesgo_todos muestra todos < 70%.';
