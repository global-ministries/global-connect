...
-- Actualiza la RPC para restringir que usuarios con rol 'Miembro' del grupo vean auditoría
...
BEGIN
  ...
  -- Usuarios con rol 'Miembro' del grupo NO pueden ver la auditoría de ese grupo
  no_miembros AS (
    SELECT b.*
    FROM base b
    WHERE NOT EXISTS (
  ...
  counted AS (
    SELECT x.*, (SELECT count(*) FROM autorizada) AS total_count
    FROM autorizada x
  )
  SELECT
    c.id,
