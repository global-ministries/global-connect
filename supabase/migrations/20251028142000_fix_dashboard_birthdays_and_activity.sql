-- Unifica correcciones:
-- - Actividad reciente con ORDER BY/LIMIT en subconsulta (evita 42803)
-- - Próximos cumpleaños: analiza todos los usuarios y normaliza año
-- - Asistencia semanal desde reporte con p_incluir_todos=true
...
BEGIN
  ...
  -- Asistencia semanal (todos los grupos activos)
  BEGIN
  ...
  -- Actividad reciente (subselect ordenado y limitado)
  v_actividad := (
    SELECT COALESCE(
  ...
  -- Próximos cumpleaños (14 días, todos los usuarios)
  v_cumpleanos := (
    WITH base AS (
  ...
  -- Tendencia y riesgo desde reporte semanal
  ...
  -- Distribución por segmento (miembros únicos, grupos activos no eliminados)
  v_distribucion := (
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'id', s.id,
