-- Restringe director-etapa a usuarios de grupos explícitamente asignados
-- Mantiene p_contexto_relacion para líderes y extiende (si en futuro) a director-etapa SOLO en contexto relaciones si queremos visibilidad amplia (por ahora no)
DROP FUNCTION IF EXISTS public.listar_usuarios_con_permisos(uuid, text, text[], boolean, boolean, boolean, int, int, boolean);
...
BEGIN
  ...
  CASE usuario_rol
    ...
    WHEN 'director-etapa' THEN
      -- Nuevo criterio: sólo usuarios miembros de grupos que el director tiene asignados en director_etapa_grupos
      query_where := format('
        WHERE u.id IN (
    ...
    WHEN 'lider' THEN
      IF p_contexto_relacion THEN
        -- En contexto de creación de relación el líder puede ver todos (mantener comportamiento ampliado)
        query_where := ' WHERE 1=1 ';
      ELSE
        query_where := format('
          WHERE u.id IN (
