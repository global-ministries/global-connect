-- Añadir campo global hay_mis_grupos (independiente de paginación) a obtener_grupos_para_usuario
-- Mantiene: estado_temporal, tipos correctos y lógica de supervisado_por_mi
...
BEGIN
  ...
  WITH base AS (
    ...
    SELECT bool_or(soy_miembro) AS hay_mis_grupos FROM base
  ), counted AS (
...
COMMENT ON FUNCTION public.obtener_grupos_para_usuario(uuid, uuid, uuid, boolean, uuid, uuid, int, int, boolean) IS 'Lista grupos visibles; incluye soy_miembro y hay_mis_grupos (global sin paginación), estado_temporal y supervisado_por_mi.';
