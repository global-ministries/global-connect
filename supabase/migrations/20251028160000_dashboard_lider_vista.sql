...
BEGIN
  ...
  ELSIF v_rol_nombre = 'lider' THEN
    ...
    v_kpis_grupo := jsonb_build_object(
      'asistencia_ultima_reunion', (
        SELECT COALESCE(ROUND((COUNT(a.id) FILTER (WHERE a.presente = true))::numeric / NULLIF(COUNT(a.id), 0)::numeric * 100, 1), 0)
        FROM public.asistencia a
        WHERE v_evento_ultimo IS NOT NULL AND a.evento_grupo_id = v_evento_ultimo
      ),
      'total_miembros', (
        SELECT COALESCE(COUNT(DISTINCT gm.usuario_id), 0)
        FROM public.grupo_miembros gm
        WHERE v_evento_ultimo_grupo IS NOT NULL AND gm.grupo_id = v_evento_ultimo_grupo AND (gm.fecha_salida IS NULL)
      )
    );
