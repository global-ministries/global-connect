-- Fix: corregir referencia a tabla inexistente director_etapa_segmento_grupos
-- La tabla real usada previamente es director_etapa_grupos + segmento_lideres
-- Re-crea la función con la firma extendida (incluye p_eliminado) sin cambiar contrato para el frontend.
...
BEGIN
  ...
  RETURN QUERY WITH base AS (
    SELECT g.*, s.nombre AS segmento_nombre, t.nombre AS temporada_nombre, g.fecha_creacion,
           p.id AS parroquia_id, p.nombre AS parroquia_nombre,
           ...
           (SELECT count(*) FROM public.grupo_miembros gm2 WHERE gm2.grupo_id = g.id) AS miembros_count,
           (
             SELECT EXISTS(
    ...
    WHERE (is_admin OR public.puede_ver_grupo(internal_user_id, g.id))
  ...
  SELECT id, nombre, activo, eliminado, segmento_nombre, temporada_nombre, fecha_creacion,
         municipio_id, municipio_nombre, parroquia_id, parroquia_nombre,
         lideres, miembros_count, supervisado_por_mi, total_count
  FROM counted
...
COMMENT ON FUNCTION public.obtener_grupos_para_usuario(uuid, uuid, uuid, boolean, uuid, uuid, int, int, boolean) IS 'Lista grupos visibles con soporte papelera y flag supervisado_por_mi.';