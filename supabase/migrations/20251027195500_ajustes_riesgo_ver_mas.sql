-- Migración: Ajustes en grupos en riesgo y listado "ver más"
-- Cambios:
-- - Excluir 0% de la lista de "Grupos que Necesitan Atención" (top 5)
-- - Agregar lista completa de grupos en riesgo (todos los < 100%)
-- - Mantener respeto a p_incluir_todos para incluir/excluir grupos sin registros
...
BEGIN
  ...
  kpis AS (
    SELECT
      ...
      -- Nuevos KPIs: asistentes y total miembros en grupos activos
  ...
  -- Por segmento (respetando switch)
  v_por_segmento := (
    WITH grupos_perm AS (
      SELECT g.id, g.segmento_id
      FROM public.grupos g
  ...
  -- Grupos en riesgo (TOP 5, excluye 0%)
  v_grupos_riesgo := (
    WITH grupos_perm AS (
      SELECT g.id
      FROM public.grupos g
  ...
  -- Grupos en riesgo (TODOS los < 100%, incluye 0% si aplica)
...
IS 'Reporte semanal de asistencia con switch p_incluir_todos. Incluye top_5_grupos_en_riesgo (sin 0%) y grupos_en_riesgo_todos (todos <100%).';
