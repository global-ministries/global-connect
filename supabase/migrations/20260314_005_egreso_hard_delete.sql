-- Cambiar egreso de soft delete a hard delete en ambas RPCs
-- El historial ya queda en historial_movimientos_grupo

-- 1. crear_solicitud_grupo: flujo directo de egreso
CREATE OR REPLACE FUNCTION public.crear_solicitud_grupo(
  p_auth_id uuid, p_tipo text, p_usuario_id uuid, p_grupo_id uuid,
  p_grupo_origen_id uuid DEFAULT NULL, p_rol_solicitado text DEFAULT NULL, p_motivo text DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
-- 2. procesar_solicitud_grupo: egreso aprobado
  ...
  IF p_accion = 'aprobar' THEN
    ...
    ELSIF v_sol.tipo = 'activacion_grupo' THEN
      UPDATE public.grupos SET 
        activo = true, estado_ciclo = 'activo', estado_aprobacion = 'aprobado'
