-- Fix: la función de KPIs comparaba p_auth_id (auth) contra columnas que almacenan usuario_id interno
-- Esto hacía que director/líder nunca coincidieran y el universo quedara vacío -> todo 0
...
DECLARE
  ...
  v_es_lider boolean;
  v_usuario_id uuid; -- id interno en public.usuarios
BEGIN
  ...
  SELECT EXISTS(
    ...
    WHERE ur.usuario_id = v_usuario_id AND r.nombre_interno IN ('admin','pastor','director-general')
  ) INTO v_es_superior;
