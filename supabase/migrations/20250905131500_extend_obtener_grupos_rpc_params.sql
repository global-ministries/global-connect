-- Agrega filtros opcionales y paginación a la RPC y expone total_count vía OUT param
...
BEGIN
  ...
  WITH base AS (
    SELECT g.*,
           s.nombre AS seg_nombre,
           t.nombre AS temp_nombre,
           m.id AS muni_id,
           m.nombre AS muni_nombre,
           p.id AS parr_id,
           p.nombre AS parr_nombre
  ...
  SELECT
    ...
    c.nombre,
    c.activo,
    c.seg_nombre AS segmento_nombre,
    c.temp_nombre AS temporada_nombre,
    c.fecha_creacion,
    c.muni_id AS municipio_id,
    c.muni_nombre AS municipio_nombre,
    c.parr_id AS parroquia_id,
    c.parr_nombre AS parroquia_nombre,
    l.lideres_data AS lideres,
    m.total_miembros AS miembros_count,
    c.total_count
  FROM counted c
