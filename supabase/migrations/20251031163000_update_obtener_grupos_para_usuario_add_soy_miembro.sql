-- Añadir campo soy_miembro a obtener_grupos_para_usuario y mantener estado_temporal y tipos correctos
  ...
  RETURN QUERY WITH base AS (
    SELECT 
      ...
      (
        SELECT EXISTS(
          SELECT 1
          FROM public.grupo_miembros gm3
...
COMMENT ON FUNCTION public.obtener_grupos_para_usuario(uuid, uuid, uuid, boolean, uuid, uuid, int, int, boolean) IS 'Lista grupos visibles; incluye soy_miembro, estado_temporal y supervisado_por_mi. Tipos consistentes.';
