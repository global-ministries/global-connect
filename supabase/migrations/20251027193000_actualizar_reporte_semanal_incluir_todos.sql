-- Migración: Actualizar obtener_reporte_semanal_asistencia para soportar incluir todos los grupos activos
-- Motivo: Permitir calcular métricas considerando también grupos sin reporte en la semana (conteo como 0%)
-- Cambios: Agrega parámetro p_incluir_todos boolean DEFAULT false y ajusta los cálculos de KPIs, segmentos, tendencia y grupos en riesgo

-- Asegurar que no exista la versión anterior con 2 parámetros para evitar ambigüedad
DROP FUNCTION IF EXISTS public.obtener_reporte_semanal_asistencia(uuid, date);
...
BEGIN
  ...
  -- 2) Fechas (ISO): semana lunes-domingo
  ...
  -- 3) Grupos activos permitidos
  -- Nota: director-etapa se filtra por director_etapa_grupos + segmento_lideres
  --       admin/pastor/director-general ven todos los activos
  --       líder/miembro no llegan aquí

  -- KPIs y comparativos de la semana
  ...
  base_grupos_semana AS (
    -- Cuando p_incluir_todos=true, incluimos todos los grupos activos aunque no tengan eventos (totales 0)
    SELECT 
      ga.id AS grupo_id,
      ga.segmento_id,
  ...
  kpis AS (
    SELECT
      ...
      CASE WHEN p_incluir_todos = false THEN
        COALESCE(
          ROUND(
            (SUM(es.total_presentes)::numeric / NULLIF(SUM(es.total_registros),0)::numeric) * 100,
            1
          ), 0
        )
      ELSE
        COALESCE(
          ROUND(
            AVG(
              CASE WHEN bgs.total_registros > 0 
                   THEN (bgs.total_presentes::numeric / bgs.total_registros::numeric) * 100
                   ELSE 0
              END
            ), 1
          ), 0
  ...
  kpis_ant AS (
    SELECT
      CASE WHEN p_incluir_todos = false THEN
        COALESCE(
          ROUND((SUM(es.total_presentes)::numeric / NULLIF(SUM(es.total_registros),0)::numeric) * 100, 1),
          0
        )
      ELSE
        COALESCE(
          ROUND(
            AVG(CASE WHEN bga.total_registros>0 THEN (bga.total_presentes::numeric / bga.total_registros::numeric) * 100 ELSE 0 END),
            1
          ),
          0
        )
  ...
  -- 4) Tendencia 8 semanas (aplica switch)
  v_tendencia := (
    WITH semanas AS (
      SELECT 
    ...
    trend AS (
      SELECT s.semana_inicio,
        CASE WHEN p_incluir_todos = false THEN
          COALESCE(
            ROUND(
              (COUNT(a.id) FILTER (WHERE a.presente = true)::numeric / NULLIF(COUNT(a.id),0)::numeric) * 100,
              1
            ),0
          )
        ELSE
          (
            SELECT COALESCE(ROUND(AVG(
              CASE WHEN epg.total_registros > 0 
                   THEN (epg.total_presentes::numeric / epg.total_registros::numeric) * 100 
                   ELSE 0 END
            ), 1), 0)
            FROM (
  ...
  v_por_segmento := (
    ...
    grupo_stats AS (
      SELECT 
        ...
        CASE WHEN COALESCE(e.total_registros,0) > 0 
             THEN (COALESCE(e.total_presentes,0)::numeric / COALESCE(e.total_registros,0)::numeric) * 100
             ELSE 0 END AS pct_grupo
      FROM grupos_perm gp
    ...
    seg AS (
      SELECT 
        segmento_id,
        CASE WHEN p_incluir_todos = true THEN
          ROUND(AVG(pct_grupo), 1)
        ELSE
          COALESCE(ROUND((SUM(total_presentes)::numeric / NULLIF(SUM(total_registros),0)::numeric) * 100, 1), 0)
        END AS porcentaje,
  ...
  -- 6) Top grupos
  -- Perfectos: solo con datos (>=1 evento) y 100%
  v_grupos_perfectos := (
    WITH grupos_perm AS (
      SELECT g.id
      FROM public.grupos g
  ...
  -- En riesgo: ordenar por porcentaje asc. Respeta el switch p_incluir_todos
  v_grupos_riesgo := (
    WITH grupos_perm AS (
      SELECT g.id
      FROM public.grupos g
    ...
    SELECT jsonb_agg(
      jsonb_build_object(
        'id', o.grupo_id,
        'nombre', g.nombre,
        'porcentaje_asistencia', o.porcentaje,
        'lideres', COALESCE(l.lideres,'Sin líderes asignados')
      )
    )
    FROM orden o
...
IS 'Reporte semanal de asistencia. p_incluir_todos=true incluye grupos activos sin reporte (0%) en KPIs, segmentos, tendencia y riesgo.';

COMMENT ON FUNCTION public.obtener_reporte_semanal_asistencia IS 'Reporte semanal de asistencia. p_incluir_todos=true incluye grupos activos sin reporte (0%) en KPIs, segmentos, tendencia y riesgo.';
