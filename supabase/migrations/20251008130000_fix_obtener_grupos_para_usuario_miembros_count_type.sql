-- Fix: asegurar tipos de columnas miembros_count (integer) y total_count (bigint) en obtener_grupos_para_usuario
-- Error previo 42804: count(*) produce bigint y se proyectó en posición de miembros_count (integer) o mismatch de orden.
-- Estrategia: castear count(*) a int explícitamente y mantener total_count como bigint.
...
BEGIN
  ...
  RETURN QUERY WITH base AS (
    SELECT 
      ...
      (SELECT count(*)::int FROM public.grupo_miembros gm2 WHERE gm2.grupo_id = g.id) AS g_miembros_count,
      (
        SELECT EXISTS(
...
COMMENT ON FUNCTION public.obtener_grupos_para_usuario(uuid, uuid, uuid, boolean, uuid, uuid, int, int, boolean) IS 'Lista grupos visibles con soporte papelera, supervisión y tipos consistentes.';
