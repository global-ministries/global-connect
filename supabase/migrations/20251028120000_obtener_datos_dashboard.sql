-- RPC unificada para dashboard (enfoque roles superiores)
-- Retorna datos agregados para widgets de administrador/pastor/director-general
...
DECLARE
  ...
  v_total_miembros_activos int := 0;
...
BEGIN
  ...
  -- Si no es superior, por ahora retornamos estructura básica vacía (se podrá extender)
  ...
  -- KPIs Globales
  SELECT COUNT(*) INTO v_total_miembros_activos FROM public.usuarios u WHERE u.activo = true;
  SELECT COUNT(*) INTO v_total_miembros_hace_30 FROM public.usuarios u
   WHERE u.activo = true AND u.fecha_creacion <= (CURRENT_DATE - INTERVAL '30 days');
  v_variacion_miembros := CASE WHEN v_total_miembros_hace_30 > 0 
    THEN ROUND(((v_total_miembros_activos - v_total_miembros_hace_30)::numeric / v_total_miembros_hace_30::numeric) * 100, 1)
    ELSE 0 END;

  -- Reusar reporte semanal (p_incluir_todos=false)
  BEGIN
    v_rep := public.obtener_reporte_semanal_asistencia(p_auth_id, NULL, false);
  ...
  v_actividad := (
    WITH eventos AS (
      SELECT u.fecha_creacion AS fecha,
             'NUEVO_MIEMBRO'::text AS tipo,
             (u.nombre || ' ' || u.apellido || ' se ha unido a la comunidad.') AS texto
      FROM public.usuarios u
      WHERE u.fecha_creacion IS NOT NULL
      UNION ALL
      SELECT g.fecha_creacion AS fecha,
             'NUEVO_GRUPO'::text AS tipo,
             ('Se creó el grupo "' || g.nombre || '".') AS texto
      FROM public.grupos g
      WHERE g.fecha_creacion IS NOT NULL
      UNION ALL
      SELECT gm.fecha_creacion AS fecha,
             'USUARIO_A_GRUPO'::text AS tipo,
      ...
      WHERE gm.fecha_creacion IS NOT NULL
      UNION ALL
      SELECT eg.fecha AS fecha,
             'REPORTE_ASISTENCIA'::text AS tipo,
  ...
  v_cumpleanos := (
    WITH base AS (
      SELECT 
        ...
        u.foto_perfil_url,
        u.fecha_nacimiento,
        -- próximo cumpleaños en el año en curso
        (CASE 
          WHEN u.fecha_nacimiento IS NULL THEN NULL
          ELSE 
            (make_date(EXTRACT(YEAR FROM CURRENT_DATE)::int, EXTRACT(MONTH FROM u.fecha_nacimiento)::int, EXTRACT(DAY FROM u.fecha_nacimiento)::int))
        END) AS cumple_este_anio
      FROM public.usuarios u
      WHERE u.fecha_nacimiento IS NOT NULL
    ), normalizado AS (
      SELECT 
        id, nombre, apellido, foto_perfil_url, fecha_nacimiento,
        CASE 
          WHEN cumple_este_anio IS NULL THEN NULL
          WHEN cumple_este_anio < CURRENT_DATE THEN (cumple_este_anio + INTERVAL '1 year')::date
          ELSE cumple_este_anio::date
    ...
    SELECT COALESCE(jsonb_agg(
      jsonb_build_object(
        ...
        'proximo', proximo_cumple
      ) ORDER BY proximo_cumple ASC
    ), '[]'::jsonb)
    FROM normalizado
    WHERE proximo_cumple BETWEEN CURRENT_DATE AND (CURRENT_DATE + INTERVAL '14 days')
    LIMIT 7
  );

  -- Grupos en riesgo y tendencia: reusar RPC semanal
  ...
  -- Distribución por segmento: miembros únicos por segmento
  v_distribucion := (
  ...
  RETURN jsonb_build_object(
    ...
    'widgets', jsonb_build_object(
      'kpis_globales', jsonb_build_object(
        'total_miembros', jsonb_build_object('valor', v_total_miembros_activos, 'variacion', v_variacion_miembros),
