-- KPI granular para grupos según rol (director-etapa: solo asignados, líder: sus grupos, superiores: global)
-- Versión inicial sin métricas de asistencia (campo placeholder)

-- Vista de soporte consolidando datos mínimos para KPIs
CREATE OR REPLACE VIEW public.v_grupos_supervisiones AS
SELECT
  g.id AS grupo_id,
  g.segmento_id,
  g.temporada_id,
  g.estado_aprobacion,
  g.created_at,
  deg.director_etapa_id,
  sl.usuario_id AS director_etapa_usuario_id,
  (
...
LEFT JOIN public.director_etapa_grupos deg ON deg.grupo_id = g.id
LEFT JOIN public.segmento_lideres sl ON sl.id = deg.director_etapa_id;
...
BEGIN
  ...
  SELECT
    total AS total_grupos,
    con_lider AS total_con_lider,
    CASE WHEN total > 0 THEN ROUND(con_lider::numeric * 100 / total, 2) ELSE 0 END AS pct_con_lider,
    aprobados AS total_aprobados,
    CASE WHEN total > 0 THEN ROUND(aprobados::numeric * 100 / total, 2) ELSE 0 END AS pct_aprobados,
    prom_miembros AS promedio_miembros,
    std_miembros AS desviacion_miembros,
    sin_director AS total_sin_director,
    CASE WHEN total > 0 THEN ROUND(sin_director::numeric * 100 / total, 2) ELSE 0 END AS pct_sin_director,
