-- Fix: agregar RETURN QUERY al SELECT principal de la versión extendida con filtros
...
CREATE FUNCTION public.obtener_grupos_para_usuario(
  ...
  p_estado_temporal text DEFAULT NULL, -- 'actual'|'pasado'|'futuro'
  p_solo_mios boolean DEFAULT false    -- true: solo grupos donde el usuario es miembro
)
RETURNS TABLE(
  id uuid,
  nombre text,
...
COMMENT ON FUNCTION public.obtener_grupos_para_usuario(uuid, uuid, uuid, boolean, uuid, uuid, int, int, boolean, text, boolean) IS 'Lista grupos con filtros por estado_temporal y solo-mios; incluye soy_miembro, soy_lider y hay_mis_grupos. RETURN QUERY corregido.';
