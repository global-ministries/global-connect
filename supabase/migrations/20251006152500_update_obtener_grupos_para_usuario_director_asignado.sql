-- Ajuste obtener_grupos_para_usuario para director de etapa: solo grupos asignados
-- Añade campo supervisado_por_mi (boolean)
-- Importante: para añadir nueva columna en RETURN TABLE debemos eliminar la versión previa;
-- esta función NO se usa en políticas RLS críticas, por lo que el DROP es seguro.
-- Si en el futuro existieran dependencias (views/policies) se deberá recrear esas dependencias después.
...
DECLARE
  ...
  es_director_etapa boolean := false;
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1 FROM public.usuario_roles ur
    JOIN public.roles_sistema rs ON ur.rol_id = rs.id
    WHERE ur.usuario_id = internal_user_id AND rs.nombre_interno = 'director-etapa'
  ...
  SELECT c.id, c.nombre, c.activo, c.seg_nombre AS segmento_nombre,
         ...
         (c.id IN (SELECT grupo_id FROM supervised)) AS supervisado_por_mi,
         c.total_count
  FROM counted c
