-- Registrar asistencia en una sola transacción
-- Crea el evento (si no existe uno para la misma fecha) y registra asistencias de miembros.
-- Seguridad: requiere que p_auth_id pueda editar el grupo (usa puede_editar_grupo)
...
create or replace function public.registrar_asistencia(
	...
	p_notas text default null,
		p_asistencias jsonb default null -- [{usuario_id, presente:boolean, motivo_inasistencia?:text}]
)
returns uuid
language plpgsql
security definer
...
begin
	...
	if v_evento_id is null then
		...
		values (gen_random_uuid(), p_grupo_id, p_fecha, p_hora, p_tema, p_notas)
		returning id into v_evento_id;
	else
		-- Actualizar metadata si llegó algo
	...
	if p_asistencias is not null then
		...
		loop
			...
			values (gen_random_uuid(), v_evento_id, v_usuario_id, v_presente, v_motivo, (
