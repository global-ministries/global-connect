...
/**
 * Dream Team capability gates — pure, client-safe.
 *
 * Split out of route-access.ts so client components (e.g. the desktop
 * sidebar) can gate on Dream Team capabilities without importing
 * createSupabaseServerClient (server-only, uses next/headers). Everything
 * here only touches PlatformSession and lib/platform/experiences.ts, both
 * free of server-only imports. route-access.ts re-exports the public
 * functions below so its existing callers are unaffected.
 */
